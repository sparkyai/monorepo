...
export default function ImageAnalyticsWidget() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/analytics/image?period=${period}`, { signal: controller.signal }).then(async (response) => {
  ...
  return (
    <Widget>
      ...
      {templates.map((role) => (
        ...
        >
          <span>{role.name}</span>
          <AnalyticsDialog url={`/api/analytics/image/templates/${role.id}`} />
        </div>
      ))}
    </Widget>
  );

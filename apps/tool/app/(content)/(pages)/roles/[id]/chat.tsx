...
export default function Chat(props: ChatProps) {
  const [history, isLoading, chat] = useChat(`/api/roles/${props.id}/completion`, [
    {
      role: "system",
      ...
      >
        ...
        {isLoading && history[history.length - 1]?.role !== "assistant" && (
          <div className="flex justify-center gap-1 py-3">
            <div className="animate-[bounce_1.2s_infinite_-.9s] py-2">
              <div className="h-2 w-2 rounded-full bg-slate-400" />
            </div>
            <div className="animate-[bounce_1.2s_infinite_-.6s] py-2">
              <div className="h-2 w-2 rounded-full bg-slate-400" />
            </div>
            <div className="animate-[bounce_1.2s_infinite_-.3s] py-2">
              <div className="h-2 w-2 rounded-full bg-slate-400" />
            </div>
            <div className="animate-[bounce_1.2s_infinite] py-2">
              <div className="h-2 w-2 rounded-full bg-slate-400" />
            </div>
          </div>
        )}

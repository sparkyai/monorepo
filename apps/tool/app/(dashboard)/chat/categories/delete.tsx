...
import { deleteChatCategory } from "@lib/actions/chat/category";
...
export default function DeleteCategory(props: DeleteCategoryProps) {
  ...
  return (
    <>
      ...
      <Dialog className="gap-2" onClose={onClose} onDelete={onDelete} open={isOpen} title="Delete Category">
        <p>
          The roles in the &ldquo;{props.category.name}&rdquo; category will also be deleted. Are you sure you want to
          delete the category?
        </p>
  ...
  function onDelete() {
    startTransition(async () => {
      const response = await deleteChatCategory(props.category.id);

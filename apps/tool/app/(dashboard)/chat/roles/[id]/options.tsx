...
export default function Options(props: OptionsProps) {
  ...
  const [topP, setTopP] = useState(props.role.parameters?.top_p || 1);
  const [model, setModel] = useState(props.role.parameters?.model || "gpt-3.5-turbo");
  const [temperature, setTemperature] = useState(props.role.parameters?.temperature || 1);
  const [presentPenalty, setPresentPenalty] = useState(props.role.parameters?.present_penalty || 0);
  const [frequencyPenalty, setFrequencyPenalty] = useState(props.role.parameters?.frequency_penalty || 0);
  ...
  const mutate = useDebouncedCallback(() => {
    ...
    void updateChatRole(props.role.id, { parameters }).then((response) => {
      if (response.error) {

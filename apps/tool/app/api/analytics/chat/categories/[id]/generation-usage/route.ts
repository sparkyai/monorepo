...
import { getChatCategoryGenerationUsage } from "@lib/data/analytics";
...
export async function GET(request: NextRequest, props: CategoryProps) {
  ...
  return NextResponse.json({ data: await getChatCategoryGenerationUsage(Number(props.params.id), period) });
}

...
import { getTextCategoryGenerationUsage } from "@lib/data/analytics";
...
export async function GET(request: NextRequest, props: CategoryProps) {
  ...
  return NextResponse.json({ data: await getTextCategoryGenerationUsage(Number(props.params.id), period) });
}

...
import { getTelegramUserGenerationUsage } from "@lib/data/analytics";
...
export async function GET(request: NextRequest, props: CategoryProps) {
  ...
  return NextResponse.json({ data: await getTelegramUserGenerationUsage(Number(props.params.id), period) });
}
